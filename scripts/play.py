...
# All rights reserved.
#
# Play script for MimicKit with Isaac Lab - Policy only mode.

"""Test/play a trained policy without reference motion."""
...
parser = argparse.ArgumentParser(description="Play a trained policy (policy only, no reference motion).")
...
def main(env_cfg: DirectRLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    """Play with RSL-RL agent (policy only, no reference motion)."""
    # Override number of environments
    env_cfg.scene.num_envs = args_cli.num_envs
    
    # Disable reference character visualization
    env_cfg.visualize_ref_char = False
    
    # Disable early termination for continuous playback
    env_cfg.enable_early_termination = False
    env_cfg.pose_termination = False
    
    print("[INFO] Mode: PLAY (Policy only, no reference motion)")
    ...
    print("[INFO] Running policy-controlled character...")
